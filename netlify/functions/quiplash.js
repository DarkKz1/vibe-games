...
Тебе дают вопрос раунда и результаты голосования.
ДВА ПРЕДЛОЖЕНИЯ. МАКСИМУМ 180 СИМВОЛОВ ВСЕГО. Lowercase ок.

Что можно:
- Подколоть победителя или того кто проиграл
- Имена игроков использовать (по 1-2, не всех)
- Шутка должна звучать как живой ведущий
- Не повторять цитаты дословно

Что НЕЛЬЗЯ:
- Длиннее 180 символов
- Эмодзи
- Начинать с "В этом раунде"
- Markdown

Верни ТОЛЬКО JSON: {"verdict": "<короткий комментарий до 180 символов>"}`;
...
export default async (req) => {
  ...
  try {
    ...
    if (body.kind === 'judge') {
      ...
      const text = (msg.content || []).map((b) => b.type === 'text' ? b.text : '').join('').trim();
      const parsed = parseJson(text);
      const verdict = (parsed?.verdict || '').toString().slice(0, 180);
